...
export default class DragManager {
    ...
    private _dragMoveNodes:Array<cc.Node> = [];
    ...
    public addDragMove(node:cc.Node){
        if(this._dragMoveNodes.indexOf(node)<0){
            this._dragMoveNodes.push(node);
        }
    }
    public removeDragMove(node:cc.Node){
        var idx =this._dragMoveNodes.indexOf(node);
        if(idx>-1){
            this._dragMoveNodes.splice(idx,1);
        }
    }
    ...
    private checkDragMove(e:cc.Event.EventTouch){
        this._dragMoveNodes.forEach((node:cc.Node)=>{
            //获取target节点在父容器的包围盒，返回一个矩形对象
            let rect:cc.Rect = node.getBoundingBox();
            //使用target容器转换触摸坐标
            let location = e.getLocation();
            let point:cc.Vec2 = node.parent.convertToNodeSpaceAR(location);
            //if (cc.rectContainsPoint(rect, targetPoint)) {
            //Creator2.0使用rect的成员contains方法
            if (rect.contains(point)) {
                node.dispatchEvent(new cc.Event(CDragEvent.DRAG_MOVE,true));
            }
        })
    }
    ...
    private dragEnd(e:cc.Event.EventTouch){
        ...
        this.checkDrop(e);
        this.dragNode.dispatchEvent(new cc.Event(CDragEvent.DRAG_END,true));
        this.reset();
